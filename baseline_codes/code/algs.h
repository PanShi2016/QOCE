void LinkComm()
{
	...
	{
		//systemCall("rm -rf " + config["RESULT_DIR"]);
		//systemCall("mkdir " + config["RESULT_DIR"]);
		...
		//systemCall("python "+config["LinkCommunity_Dir"]+"edgelist2pairs.py -o "+config["DATA_DIR"]+config["INSTANCE_NAME"]+"_lc "+config["TMP_DIR"]+config["INSTANCE_NAME"]+"_lc");
		...
		//getchar();
		//systemCall("rm "+config["TMP_DIR"]+"*.*");
		//systemCall("rm ./"+config["INSTANCE_REMOVE"]+"*");
	...
	else
	{
		...
		//systemCall("python "+config["LinkCommunity_Dir"]+"edgelist2pairs.py -o "+config["DATA_DIR"]+config["INSTANCE_NAME"]+"_lc "+config["TMP_DIR"]+config["INSTANCE_NAME"]+"_lc");
	    	//systemCall("python "+config["LinkCommunity_Dir"]+"link_clustering_update.py -d \" \" " + config["DATA_DIR"]+config["INSTANCE_NAME"]+"_lc");
		...
		//systemCall("python "+config["LinkCommunity_Dir"]+"link_clustering_update.py -d \" \" " + config["DATA_DIR"]+"graph_single.pairs");
...
void DEMONComm()
{
	...
	//systemCall("python "+config["LinkCommunity_Dir"]+"edgelist2pairs.py -o "+config["DATA_DIR"]+config["INSTANCE_NAME"]+"_lc "+config["TMP_DIR"]+config["INSTANCE_NAME"]+"_lc");
	//systemCall("mv -f ./" +config["INSTANCE_NAME"]+"_lc "+ config["RESULT_DIR"]);
	struct timeval start,end;
    	gettimeofday(&start,NULL);

	systemCall("python " + config["DEMON_Dir"] + "launch.py "+config["DATA_DIR"]+config["INSTANCE_NAME"]);
	//getchar();
	//systemCall("python " + config["DEMON_Dir"] + "launch.py "+config["DATA_DIR"]+config["INSTANCE_NAME"]+"_lc");
	//systemCall("./code/dataTransback DEMON.gen DEMONnew.gen");
	...
	//systemCall("mv -f ./DEMONnew.gen " + config["RESULT_DIR"]+"DEMON.gen");
	//systemCall("mv -f ./Demon_time.txt " + config["RESULT_DIR"]+"Demon_time.gen");
...
void CFinderComm()
{
	//systemCall("rm -r "+config["TMP_DIR"]+config["INSTANCE_NAME"]+"*");
	systemCall("rm -r "+config["TMP_DIR"]); //删除临时目录 否则提示ERROR: the output directory already exists and it is non-empty.
	struct timeval start,end;
    	gettimeofday(&start,NULL);
	...
	//systemCall("rm ./CFinder_time.txt ");
	//systemCall("mv ./Demon_time.txt " + config["RESULT_DIR"]);
...
void OSLOMComm()
{
	...
	//systemCall("mv -f ./OSLOM_time.txt " + config["RESULT_DIR"]);
...
void NISEComm()
{
	...
	string NISEGraphFile=config["TMP_DIR"]+config["INSTANCE_NAME"];
  	systemCall("matlab2015 -nodesktop -nosplash -r \"addpath('"+config["NISE_Dir"]+"');addpath('"+config["NISE_Dir"]+"matlab_bgl');addpath('"+config["NISE_Dir"]+"GraclusSeeds/graclus1.2/matlab');"+"NISERun('"+NISEGraphFile+"','"+config["NISE_seeds"]+"',"+config["NISE_k"]+");exit\"");
	systemCall("cd ../..");
	gettimeofday(&end,NULL);
	double span = end.tv_sec-start.tv_sec + (end.tv_usec - start.tv_usec)/1000000.0;
    	ofstream otime;
...
void BigClamComm()
{
	...
	systemCall(config["BigClam_Dir"] + "dataTran " + config["TMP_DIR"]+config["INSTANCE_NAME"] + " " + config["TMP_DIR"]+"graph.txt");
  	...
  	//systemCall(config["BigClam_Dir"] + "bigclam" + " -i:" + config["TMP_DIR"]+"graph.txt" + " -o:" + config["RESULT_DIR"] + "BigClam.gen "+ "-c:"+config["BigClam_c"] + " | tee " + config["RESULT_DIR"] + "BigClam.log");
